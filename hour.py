...
output_csv = 'processed_data317.csv'   # 输出文件路径
...
if len(numeric_df.columns) > 0:
    z_scores_before = numeric_df.apply(zscore)
...
for i in range(0, len(df), group_size):
    ...
    # 只计算数值列的平均值
    group_mean = group.select_dtypes(include=['number']).mean()
...
# 处理后的异常值检测
# 确保参与计算的列是数值类型
numeric_result_df = result_df.select_dtypes(include=['number'])
if len(numeric_result_df.columns) > 0:
    z_scores_after = numeric_result_df.apply(zscore)
